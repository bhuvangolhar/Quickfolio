...
// API Base URL (with localhost fallback for local development)
...
// WeakMap with explicit config typing to track retry attempts
const retryConfig = new WeakMap<InternalAxiosRequestConfig, number>();

// Request Interceptor: Attach authentication token & legacy URL admin_key
api.interceptors.request.use((config) => {
  if (typeof window !== "undefined") {
    // Legacy support: extract admin_key from URL search params
    const queryKey = new URLSearchParams(window.location.search).get("admin_key");
    if (queryKey) {
      config.params = { ...config.params, admin_key: queryKey };
    }

    // Modern auth: attach Bearer token from localStorage if present
...
// Response Interceptor: Exponential backoff retries for transient network/server failures
api.interceptors.response.use(
  ...
  async (error: AxiosError) => {
    ...
    const shouldRetry = (isNetworkError || isServerError) && retryCount < maxRetries;
