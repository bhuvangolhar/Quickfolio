...
// Custom hook for element intersection detection with optimized unobserve
function useInView<T extends HTMLElement>(ref: RefObject<T | null>, threshold = 0.15) {
...
export default function Socials({ adminMode = false }: { adminMode?: boolean }) {
  ...
  const handleCreate = async () => {
    if (!newSocial.platform.trim() || !newSocial.url.trim()) {
      setStatus("Please enter both platform name and URL.");
      return;
    }
    try {
      await createSocial(newSocial);
      setStatus("Social created successfully.");
  ...
  const handleUpdate = async (id: number) => {
    ...
    try {
      await updateSocial(id, payload);
      setStatus("Social updated successfully.");
      window.location.reload();
    } catch {
  ...
  const handleDelete = async (id: number) => {
    try {
      ...
      setStatus("Social deleted successfully.");
      window.location.reload();
    } catch {
  ...
  const bioLines = profile?.contact_bio ? profile.contact_bio.split("\n") : [];

  return (
    <section
      id="socials"
      className="relative overflow-hidden min-h-screen flex flex-col items-center justify-center pb-4"
    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-6 w-full" ref={sectionRef}>
        <div className="max-w-2xl mx-auto text-center">
          ...
          {/* Admin Controls */}
          {adminMode && (
            <div className="mb-6 p-4 border border-dashed border-amber-300/60 rounded text-left">
              <div className="mb-2 text-sm text-amber-300 font-medium">Admin: Edit Bio</div>
              <textarea
                value={editBio}
                onChange={(e) => setEditBio(e.target.value)}
                placeholder="Contact section bio text"
                rows={3}
                className="w-full px-2 py-1 mb-2 rounded border border-white/20 bg-slate-900 text-white focus:outline-none focus:border-amber-400"
              />
              <button
                onClick={handleBioUpdate}
                className="px-4 py-2 rounded bg-amber-400 text-black hover:bg-amber-300 transition-colors mb-4 font-medium"
              >
                Update Bio
              </button>
              {status && <div className="text-xs text-cyan-300 mb-4">{status}</div>}

              <div className="text-sm text-amber-300 mb-2 font-medium">Add Social</div>
              <div className="text-xs text-gray-400 mb-3 bg-slate-900/50 p-2 rounded">
                Platform examples: github, linkedin, twitter, instagram, email, phone, whatsapp
                <br />
                For phone/whatsapp: use phone number (e.g., "1234567890" or "+1-234-567-8910")
                <br />
              ...
              <input
                ...
                onChange={(e) =>
                  setNewSocial((s) => ({ ...s, platform: e.target.value }))
                }
              ...
              <button
                ...
                className="px-4 py-2 rounded bg-amber-400 text-black hover:bg-amber-300 transition-colors font-medium"
              >
                Add Social
              </button>
            </div>
            ...
            >
              {(socials ?? []).map((social, i) => {
                ...
                return adminMode ? (
                  <div
                    key={social.id}
                    className={`group flex flex-col items-start gap-2.5 p-4 border border-dashed border-amber-400/40 rounded-xl text-white transition-all duration-300 ${
                      inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
                    }`}
                    style={{ transitionDelay: `${i * 60}ms` }}
                  >
                    <input
                      className="w-full p-2 rounded border border-white/20 bg-slate-900 text-white focus:outline-none focus:border-amber-400"
                      value={editSocials[social.id]?.platform ?? social.platform}
                      onChange={(e) =>
                        setEditSocials((p) => ({
                          ...p,
                          [social.id]: { ...p[social.id], platform: e.target.value },
                        }))
                      }
                    />
                    <input
                      ...
                      onChange={(e) =>
                        setEditSocials((p) => ({
                          ...p,
                          [social.id]: { ...p[social.id], url: e.target.value },
                        }))
                      }
                    />
                    <div className="flex gap-2 w-full">
                      <button
                        ...
                        className="flex-1 py-2 rounded bg-amber-400 text-black font-medium hover:bg-amber-300 transition-colors"
                      >
                        Save
                      </button>
                      <button
                        onClick={() => handleDelete(social.id)}
                        className="flex-1 py-2 rounded bg-red-500 text-white font-medium hover:bg-red-600 transition-colors"
                      >
                        Delete
                      </button>
                    </div>
        ...
        <div className="mt-16 pt-8 flex flex-col items-center justify-center gap-6">
          ...
          <div className="mt-4">
            <span
              className="text-xs font-mono tracking-wider text-amber-400 text-center inline-block"
