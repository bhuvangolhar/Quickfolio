...
function ProjectCard({
  ...
  const handleTechChange = (idx: number, value: string) => {
    const updated = [...techs];
    updated[idx] = value;
    setEditData({ ...editData, techStack: updated.filter(Boolean).join(", ") });
  };

  const handleAddTech = () => {
    if (techs.length < 5) {
      const updated = [...techs, "New Tech"];
      setEditData({ ...editData, techStack: updated.join(", ") });
    }
  };

  const handleRemoveTech = (idx: number) => {
    const updated = techs.filter((_, i) => i !== idx);
    setEditData({ ...editData, techStack: updated.join(", ") });
  };
