...
function sortByOrder<T extends { order?: number; id?: number }>(items: T[]): T[] {
...
export default function Certifications({ adminMode = false }: { adminMode?: boolean }) {
  const { data: certs, loading, error, refetch } = useFetch(fetchCertifications);
